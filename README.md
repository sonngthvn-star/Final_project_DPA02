...
* **Bronze Layer:** Raw JSON ingestion from OpenWeatherMap Air Pollution API via `scraper_bronze.py`.

* **Silver Layer:** Data cleaning, unit conversion, and toxic gas normalization stored in PostgreSQL.

* **Gold Layer:** Aggregated views and `gold_latest_snapshot` optimized for real-time dashboarding
...
#### Phase A: Environment Configuration
    ...
    3.	Add your OpenWeatherMap API Key to `OWM_API_KEY`.
...
### 📊 5. Key Features
    •	**Full ETL Automation:** Automated flow from API fetching to Silver-layer validation orchestrated by Airflow. Seamless transition from OWM API to Gold-layer views.
    
    •	**Medallion CRUD:** Ability to Update and Delete records in the Silver-layer directly via the "Data Management" table.
    
    •	**Geospatial Mapping:** Interactive Leaflet.js map with color-coded markers by the Pollutant Standard Index for global city AQI levels.
    
    •	**Professional Logging:** Centralized logger_config.py tracking all pipeline events in the /logs directory.     

    •	**Interactive Dashboard:** Real-time data visualization with Highcharts and Chart.js for dynamic insights.

    •	**Toxic Gas Tracking:** Specialized monitoring for $CO$, $NO_2$, $O_3$, and $SO_2$ concentrations.
